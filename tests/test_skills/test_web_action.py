...
async def test_web_action_write_needs_approval(sample_context):
    ...
    msg = IncomingMessage(
        ...
        text="submit order on Amazon",
    )
    with patch("src.skills.web_action.handler.approval_manager") as mock_approval:
...
async def test_web_action_read_executes_directly(sample_context):
    ...
    msg = IncomingMessage(
        ...
        chat_id="c1",
        type=MessageType.text,
        text="check what time Costco closes",
    )
    with patch("src.skills.web_action.handler.browser_tool") as mock_browser:
