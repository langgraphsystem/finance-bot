...
- query_report: генерация PDF-отчёта С ФИНАНСОВЫМИ ДАННЫМИ из БД ("отчёт", "report", \
"покажи итоги", "сгенерируй отчёт", "месячный отчёт", \
"сделай pdf расходов", "pdf сканов расходов", "отчёт по расходам в pdf", \
"expense report pdf", "скачать отчёт", "выгрузи расходы", "финансовый отчёт в pdf"). \
НЕ использовать если пользователь описывает КОНТЕНТ/ТЕКСТ документа (тему, абзацы, слова) — \
это generate_document. НЕ использовать для слова "PDF" без финансового контекста.
...
- tax_estimate: быстрая оценка налогов текущего квартала ("сколько налогов?", \
"tax estimate", "quarterly taxes", "how much do I owe in taxes?", "налоговая оценка")
- tax_report: полный налоговый отчёт с вычетами, Schedule C и PDF ("полный отчёт по налогам", \
"tax report", "generate tax report", "налоговый отчёт за год", "tax report for 2025", \
"full tax report", "schedule C", "полный налоговый анализ", "налоги за год"). \
Извлеки tax_year: год (2025, 2026...) если указан, tax_quarter: квартал (1-4) если указан \
(None = годовой отчёт, не квартальный)
...
"поставь напоминание трекеру", "поставь напоминание трекеру воды", \
"поставь напоминание трекеру привычки в 21:00", "поставь напоминание трекеру сна"). \
...
- delete_event: удалить/отменить событие из календаря ("delete the meeting", \
"удали мероприятие из календаря", "cancel my dentist appointment", \
"убери событие Поход в Чикаго", "отмени встречу", "remove event from calendar")
...
КРИТИЧНО — query_report vs generate_document (проверяй для любого "pdf"/"документ"):
- "сгенерируй pdf файл и напиши [текст]" / "создай документ [тема]" / "сделай pdf [контент]" \
  → generate_document (пользователь описывает текст/контент документа)
- "отчёт за месяц", "pdf расходов", "финансовый отчёт" → query_report (финансовые данные из БД)
- Признак generate_document: пользователь указывает что НАПИСАТЬ ВНУТРИ (тему, текст, слова)
- Признак query_report: нужны данные из БД (расходы, доходы, транзакции)
...
- "delete/cancel/удали/отмени" + событие/встреча/календарь → delete_event
...
Правила приоритета (tax_estimate vs tax_report):
- "полный отчёт", "full report", "за год", "schedule C", "PDF" → tax_report
- "сколько налогов", "quick estimate", "quarterly" без слова "report" → tax_estimate
...
def _rule_based_memory_forget(text: str) -> IntentDetectionResult | None:
    ...
    # Skip if this looks like a calendar event deletion — let delete_event handle it
    if any(hint in lower for hint in _CALENDAR_EVENT_HINTS):
        return None
...
def _rule_based_delete_event(text: str) -> IntentDetectionResult | None:
    """Fast-path direct calendar deletion requests."""
    stripped = text.strip()
    lower = stripped.lower()
    if not lower:
        return None
    if any(marker in lower for marker in _LIST_BOOKINGS_MARKERS):
        return None
    if any(marker in lower for marker in _DELETE_EVENT_MARKERS) or (
        any(verb in lower for verb in DELETE_VERBS)
        and any(hint in lower for hint in _CALENDAR_EVENT_HINTS)
    ):
        return IntentDetectionResult(
            intent="delete_event",
            confidence=0.96,
            intent_type="action",
            data=IntentData(event_title=strip_forget_command(stripped) or None),
            response=None,
        )
    return None
...
async def detect_intent(
    ...
    delete_event_fast_path = _rule_based_delete_event(text)
    if delete_event_fast_path:
        return delete_event_fast_path
...
SCOPED_INTENT_DEFS: dict[str, dict[str, str]] = {
    ...
    "finance_specialist": {
        ...
        "tax_estimate": 'быстрая оценка налогов ("quarterly taxes", "сколько налогов?")',
        "tax_report": 'полный налоговый отчёт с PDF ("tax report", "налоговый отчёт за год")',
    ...
    "calendar": {
        ...
        "delete_event": 'удалить событие ("delete meeting", "удали событие из календаря")',
...
async def detect_intent_scoped(
    ...
    delete_event_fast_path = _scoped_fast_path(_rule_based_delete_event(text))
    if delete_event_fast_path:
        return delete_event_fast_path
