...
- query_report: генерация PDF-отчёта ("отчёт", "report", "покажи итоги", "PDF", "сгенерируй отчёт", "месячный отчёт")
...
- mark_paid: отметить груз как оплаченный ("оплатили груз", "mark paid", "груз оплачен", "получил оплату за рейс")
- onboarding: первый контакт, знакомство
- set_budget: установить бюджет или лимит ("бюджет на продукты 30000", "лимит 5000 в неделю", "set budget")
- add_recurring: добавить регулярный платёж ("подписка", "recurring", "каждый месяц плачу", "аренда 50000")
- complex_query: сложный аналитический запрос ("сложный анализ", "сравни с бюджетом", "полный отчёт", "анализ трат за 3 месяца", "что происходит с финансами")
...
async def detect_intent(
    ...
    categories_str = ""
    if categories:
        categories_str = "\n".join(f"- {c.get('name', '')} ({c.get('scope', '')})" for c in categories)

    user_prompt = f"Категории пользователя:\n{categories_str}\n\nСообщение: {text}" if categories_str else f"Сообщение: {text}"
