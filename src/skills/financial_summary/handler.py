...
class FinancialSummarySkill:
    ...
    async def execute(
        ...
        categories = await self._get_category_breakdown(
            family_id, start_date, end_date, role=context.role, user_id=context.user_id
        )
        # Fetch top merchants
        merchants = await self._get_top_merchants(
            family_id, start_date, end_date, role=context.role, user_id=context.user_id, limit=5
        )
        # Fetch income total
        ...
        prev_categories = await self._get_category_breakdown(
            family_id, prev_start, start_date, role=context.role, user_id=context.user_id
        )
    ...
    async def _get_category_breakdown(
        family_id: str, start: date, end: date, role: str = "owner", user_id: str = "",
    ) -> list[dict[str, Any]]:
        ...
        async with async_session() as session:
            ...
            rows = (
                await session.execute(apply_visibility_filter(stmt, Transaction, role, user_id))
            ).all()
            return [
                {
                    "category": r.category,
