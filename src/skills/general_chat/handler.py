...
CHAT_SYSTEM_PROMPT = """Ты — AI Assistant для финансов и жизни в Telegram.
...
<b>📅 Календарь:</b> расписание, создать событие, перенести, утренняя сводка
<b>📝 Задачи:</b> дела, напоминания, списки покупок
<b>🍽 Трекинг:</b> еда, напитки, настроение, заметки, план дня, рефлексия
<b>🔍 Поиск:</b> вопросы, интернет, сравнение, карты, YouTube
<b>✍️ Тексты:</b> письма, посты, перевод, проверка грамматики
<b>📋 Клиенты:</b> бронирования, контакты, отправка сообщений
<b>🌐 Мониторинг:</b> цены, новости, действия на сайтах
<b>🗑 Данные:</b> удаление записей за период или по типу
...
- Если запрос связан с функцией AI Assistant, можешь в конце ответа мягко \
    ...
    def _get_dosing_prompt(self, suppress: bool) -> str:
        ...
        if suppress:
            return CHAT_SYSTEM_PROMPT.replace(
                ...
                "НЕ добавляй подсказки о возможностях AI Assistant "
        ...
        if assembled:
            ...
            if suppress_suggestions:
                sys = assembled.system_prompt.replace(
                    ...
                    "НЕ добавляй подсказки о возможностях AI Assistant — пользователь уже знает",
                )
            else:
